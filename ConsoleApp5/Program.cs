...
var rules = new Dictionary<KeyValuePair<char, Move>, Move>
{
    { new('F', Move.Down), Move.Left },
    { new('F', Move.Right), Move.Up },
    { new('-', Move.Left), Move.Left },
    { new('-', Move.Right), Move.Right },
    { new('7', Move.Left), Move.Up },
    { new('7', Move.Down), Move.Right },
    { new('|', Move.Up), Move.Up },
    { new('|', Move.Down), Move.Down },
    { new('J', Move.Up), Move.Right },
    { new('J', Move.Left), Move.Down },
    { new('L', Move.Right), Move.Down },
    { new('L', Move.Up), Move.Left },
};

Console.WriteLine(GetFarthestPosition(@"F7---|FJ-L-|7-7F-7.F7|-|F-L7F7--7F---FJ7-F-F.FF.J---|77--7FF-7.-7-|.7FF-77F|77.|7-FF7FJFJ7-7FF77L77F7-F.|FFJ.-|-77.JFF-|J-|F|77F77.F7-F7-FF7
...
Console.WriteLine(GetFarthestPosition1(@"
.....
.S-7.
.|.|.
.L-J.
...
int GetFarthestPosition1(string mapStr)
{
    int result = 0;
    ...
    for (int i = 0; i < rows.Length; i++)
    {
        ...
        {
            startY = i;
            startX = rows[i].IndexOf("S");
        }
        map[i] = rows[i].Trim().ToArray();
    }


    var routs = new List<(KeyValuePair<int, int> point, Move moveFrom, int score)>();
    var pastPoints = new HashSet<KeyValuePair<int, int>>();
    ...
    if (startX != 0
        ...
        || map[startY][startX - 1] == '-'))
    {
        var point = new KeyValuePair<int, int>(startY, startX - 1);
        routs.Add(new(point, Move.Right, 0));
    }
    else if (startX != map[0].Length - 1
        && (map[startY][startX + 1] == '7'
        || map[startY][startX + 1] == 'J'
        || map[startY][startX + 1] == '-'))
    {
        var point = new KeyValuePair<int, int>(startY, startX + 1);
        routs.Add(new(point, Move.Left, 0));
    }
    else if (startY != 0
        && (map[startY - 1][startX] == '|'
    ...
    {
        var point = new KeyValuePair<int, int>(startY - 1, startX);
        routs.Add(new(point, Move.Down, 0));
    }
    else if (startY != map.Length - 1
        && (map[startY + 1][startX] == 'L'
        || map[startY + 1][startX] == 'J'
        || map[startY + 1][startX] == '|'))
    {
        var point = new KeyValuePair<int, int>(startY + 1, startX);
        routs.Add(new(point, Move.Up, 0));
    }
    ...
    while (routs.Any())
    {
        ...
        {
            var currPoint = routs[i];
            currPoint.moveFrom = rules[new KeyValuePair<char, Move>(map[currPoint.point.Key][currPoint.point.Value], currPoint.moveFrom)];
            currPoint.point = GetNextPoint(currPoint.moveFrom, currPoint.point);
            currPoint.score++;
            char pointCh = map[currPoint.point.Key][currPoint.point.Value];
            if (pointCh == 'S' || pastPoints.Contains(currPoint.point))
            {
                routs.RemoveAt(i);
                pastPoints.Add(currPoint.point);
                break;
            }
            pastPoints.Add(currPoint.point);
            routs[i] = currPoint;
    ...
    //var points = pastPoints.Select(x => new Point(x.Value, x.Key)).ToList();
    //points.Add(points[0]);

    /////var area = PolygonArea(points.ToArray());
    //var area = Math.Abs(points.Take(points.Count - 1)
    //   .Select((p, i) => (points[i + 1].X - p.X) * (points[i + 1].Y + p.Y))
    //   .Sum() / 2);

    var points = pastPoints.Select(x => new KeyValuePair<int, int>((int)(x.Value * 0.5), (int)(x.Key * 0.5))).Distinct().ToList();
    points.Add(points[0]);

    ///var area = PolygonArea(points.ToArray());
    var area = Math.Abs(points.Take(points.Count - 1)
       .Select((p, i) => (points[i + 1].Key - p.Key) * (points[i + 1].Value + p.Value))
       .Sum() / 2);
...
double PolygonArea(Point[] polygon)
{
    ...
    for (i = 0; i < polygon.Length; i++)
    {
        ...
        area += (polygon[i].X + (polygon[i].X < polygon[j].X ? 1 : -1)) * (polygon[j].Y + (polygon[i].Y < polygon[j].Y ? -1 : 1));
        area -= (polygon[i].Y + (polygon[i].Y < polygon[j].Y ? 1 : -1)) * (polygon[j].X + (polygon[i].X < polygon[j].X ? -1 : 1));
    }
    ...
    int GetFarthestPosition(string mapStr)
{
    int result = 0;
    ...
    while (routs.Any())
    {
        for (int i = 0; i < routs.Count; i++)
        {
            ...
            currPoint.score++;
            char pointCh = map[currPoint.point.Key][currPoint.point.Value];
            if (pointCh == '.' || pastPoints.Contains(currPoint.point))
            {
                routs.RemoveAt(i);
                result = currPoint.score > result ? currPoint.score : result;
                i--;
                continue;
            }
            pastPoints.Add(currPoint.point);
...
KeyValuePair<int, int> GetNextPoint(Move moveFrom, KeyValuePair<int, int> oldPoint)
{
    return moveFrom switch
    {
        Move.Left => new KeyValuePair<int, int>(oldPoint.Key, oldPoint.Value + 1),
        Move.Right => new KeyValuePair<int, int>(oldPoint.Key, oldPoint.Value - 1),
        Move.Up => new KeyValuePair<int, int>(oldPoint.Key + 1, oldPoint.Value),
        Move.Down => new KeyValuePair<int, int>(oldPoint.Key - 1, oldPoint.Value),
