...
namespace ConsoleApp5
{
    internal class Class2
    {
        void Run()
        {
            Console.WriteLine(GetPossibleGamesSum1(@"Game 1: 10 red, 7 green, 3 blue; 5 blue, 3 red, 10 green; 4 blue, 14 green, 7 red; 1 red, 11 green; 6 blue, 17 green, 15 red; 18 green, 7 red, 5 blue
...
Game 100: 16 red, 3 blue; 2 red, 5 green; 9 red; 1 blue, 3 green, 10 red; 1 red, 5 blue, 3 green; 12 blue, 9 red", new Dictionary<string, int>
{
    { "red", 12 },
    { "green", 13 },
    { "blue", 14 }
            ...
            Console.WriteLine(GetPossibleGamesSum1(@"Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green", new Dictionary<string, int>
{
    { "red", 12 },
    { "green", 13 },
    { "blue", 14 }
        ...
        int GetPossibleGamesSum1(string gameMapStr, Dictionary<string, int> existCubs)
        {
            int result = 0;

            List<string[]> gamesArr = gameMapStr.Split('\n').Select(x => x.Split(':')[1].Split(',', ';')).ToList();
            string[] gamesArr1 = gameMapStr.Split("\r\n").Select(x => x.Split(':')[1]).ToArray();

            for (int i = 0; i < gamesArr1.Length; i++)
            {
                string[] values = gamesArr1[i].Split(',', ';', ' ').Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();

                Dictionary<string, int> requiredCubes = new();
                for (int j = 0; j < values.Length; j += 2)
                {
                    if (requiredCubes.ContainsKey(values[j + 1]))
                    {
                        int cubesCnt = int.Parse(values[j]);
                        if (requiredCubes[values[j + 1]] < cubesCnt)
                        {
                            requiredCubes[values[j + 1]] = cubesCnt;
                        }
                    }
                    else
                    {
                        requiredCubes.Add(values[j + 1], int.Parse(values[j]));
                    }
                }

                result += requiredCubes.Values.Aggregate((x, y) => x * y);
            }
        ...
        int GetPossibleGamesSum(string gameMapStr, Dictionary<string, int> existCubs)
        {
            int result = 0;
            ...
            for (int i = 0; i < gamesArr1.Length; i++)
            {
                ...
                for (int j = 0; j < values.Length; j += 2)
                {
                    if (existCubs.ContainsKey(values[j + 1]) && existCubs[values[j + 1]] < int.Parse(values[j]))
                    {
                        invalidGame = true;
                    }
                }
